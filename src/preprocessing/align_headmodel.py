...
def make_trans_from_coordinates(
    ...
    overwrite
        If ``True`` and ``out_trans_path`` already exists, overwrite it instead of
        raising a :class:`FileExistsError`.
    ...
    out_trans_path.parent.mkdir(parents=True, exist_ok=True)
    if out_trans_path.exists() and not overwrite:
        raise FileExistsError(
            f"Destination file exists: {out_trans_path}. Pass overwrite=True to replace it."
        )

    mne.write_trans(str(out_trans_path), trans, overwrite=overwrite)
