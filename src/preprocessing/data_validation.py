    ...
    def run_full_validation(self, subject: str, session: str, task: str,
        ...
        if validate_lfp:
            try:
                ...
                logger.warning(f"⚠ 未找到LFP数据: {e}")
                logger.info("将只验证EEG数据")
                validate_lfp = False
        
        # 执行各项检查
        results = {}
        
        if validate_lfp and has_lfp:
            # 完整的EEG-LFP验证
            results = {
            ...
            results = {
                'eeg_info': {
                    ...
                    'duration': eeg_raw.times[-1]
                },
                'eeg_events': {
                    'has_events': 'events' in eeg_metadata,
                    'event_count': len(eeg_metadata.get('events', [])) if 'events' in eeg_metadata else 0
                },
                'mode': 'eeg_only'
            }
            ...
            # 总结
            logger.info("="*60)
            logger.info("验证完成（仅EEG）")
            logger.info("="*60)
            
            all_passed = True  # EEG数据加载成功即通过
